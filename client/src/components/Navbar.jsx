...
import { IconButton, InputBase, Toolbar, useTheme, AppBar, Button, Typography, Box, Menu, MenuItem } from '@mui/material';
        ...
        <FlexBetween gap='1.5rem'>
          ...
          <FlexBetween>
            <Button onClick={handleClick} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', textTransform: 'none', gap: '1rem' }}>
              ...
              <Box textAlign='left'>
                <Typography fontWeight='bold' fontSize='.85rem' sx={{ color: theme.palette.secondary[100] }}>
                  {user.name}
                </Typography>
                <Typography fontSize='.75rem' sx={{ color: theme.palette.secondary[200] }}>
                  {user.occupation}
                </Typography>
              </Box>
